...
class Exportar extends MySQL
{
  ...
  function ConsultarContratos(){
    ...
    $contratos = parent::consulta("SELECT id_contrato FROM semaforo WHERE id_mes = '3' GROUP BY id_contrato");
    $lista_contratos = array();
    while($row = parent::fetch_assoc($contratos))
    {
      $id_contrato = $row["id_contrato"];
      $contrato = $row["id_contrato"];
      $lista_contratos[$id_contrato] = $contrato;
    }
  ...
  function ExportarContratos(){
    ...
    // AND semaforo.id_mes = 5 AND semaforo.id_contrato = '".$this->code."'
    // GROUP BY semaforo.id_tema
    // ORDER BY tema.nombre_tema");
    //
    // $tabla_informe = parent::consulta("
    ...
    // and acta.fecha_evaluacion <= now() and acta.id_contrato = '".$this->code."'
    // GROUP BY  componente.id_componente, mes
    // ORDER BY mes asc");
    //
    // while ($row = parent::fetch_assoc($semaforo)) {
    ...
    // header("Content-Disposition: attachment;filename='$this->code.docx'");
    //
