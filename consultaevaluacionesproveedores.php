...
if ($_SESSION['login'])
{
  ...
  acta_proveedor.porc_inicial,
  acta_proveedor.porc_final,
  acta_proveedor.estado
  FROM
  acta_proveedor,proveedor,modalidad,componente
  WHERE
  acta_proveedor.id_proveedor=proveedor.id_proveedor and
  acta_proveedor.id_modalidad=modalidad.id_modalidad and
  acta_proveedor.id_componente=componente.id_componente
  order by acta_proveedor.fecha_evaluacion desc
  "),$conexion);
  ...
  <head>
    ...
    <title>Consulta de Actas de Proveedores</title>
...
<div class="container">
  <div class="jumbotron">
    <h2>Creación de Actas de Proveedores</h2>
    <h5>Diligencie nuevas actas de interventoría a proveedores. Opción válida solo para el componente de Nutrición</h5>


    <p><a class="btn btn-lg btn-block btn-pascual" href="configevaluacionesproveedores.php" role="button">Realizar nueva Interventoría</a></p>
  ...
  <table align="center" class="table table-bordered table-hover" id='table' style="width: 95%">
    ...
    <tbody>
      ...
      while($row = mysql_fetch_assoc($actas_creadas)){ ?>
        <tr>
          <td class="active"><a  href='imprimiractasproveedor.php?id_acta=<?php echo $row['id_acta'] ?>&msg=0'><?php echo $row['id_acta'] ?></a></td>
          <td class="active"><?php echo $row['fecha_evaluacion'];  ?></td>
          <td class="active"><?php echo $row['nombre_componente'];  ?></td>
          <td class="active"><?php echo $row['nombre_proveedor'];  ?></td>
          <td class="active"><?php echo $row['direccion_proveedor'];  ?></td>
