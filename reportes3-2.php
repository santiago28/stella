...
<?php
 for ($id_tema = 401; $id_tema<403; $id_tema++) { ?>
                ...
                <tr>
                  ...
                  <th class="info">incumplimiento_grave</th>
                  <th class="info">descripcion_incumplimiento_grave</th>
                  <th class="info">logros_prestador</th>
                  <th class="info">dificultades_prestador</th>
                  <th class="info">debido_proceso</th>
                  <th class="info">asistencia_tecnica</th>
                  <th class="info">otras_observaciones</th>
                  </tr>
                </thead>
                <tbody>
                <?php
                    $sqlnivel1 = "SELECT s.id_semaforo, s.id_contrato, s.id_prestador, p.nombre_prestador, s.id_modalidad, m.nombre_modalidad, s.id_componente, c.nombre_componente, s.id_tema, i.id_interventor, s.id_mes, s.incumplimiento_grave,s.descripcion_incumplimiento_grave, s.logros_prestador, s.dificultades_prestador, s.debido_proceso, s.asistencia_tecnica, s.otras_observaciones
                        FROM semaforo s
                    ...
                    while ($nivel1 = mysql_fetch_assoc($respnivel1)) {
                        $sumac=0; $sumat=0;$castigo=0;$aux=0; $logros="";$dificultad="";$observ="";$texto="";
                        echo '<tr>';
                        echo '<td class="active">'.$nivel1['id_semaforo'].'</td>';
                        echo '<td class="active">'.$nivel1['id_contrato'].'</td>';
                        ...
                        $sqlnivel2="SELECT ca.idCriterio AS numero, cr.criterio AS pregunta, ca.evaluacion AS respuesta, cr.porcentaje AS porcentaje, ca.comentario, ca.tipocoment, cr.tipo, cr.excluir FROM calificacion ca INNER JOIN criterio cr ON (ca.idCriterio=cr.id) WHERE ca.idSemaforo =".$nivel1['id_semaforo']." AND cr.idTema=".$id_tema." AND ca.idMes=".$id_mes;
                        ...
                        while($nivel2 = mysql_fetch_assoc($respnivel2)) {
                            ...
                            $restot[$nivel2['numero']] = str_replace("&nbsp;", "",trim($nivel2['comentario']));
                            switch ($nivel2['tipocoment']) {
                                case 'Logro':
                                    $logros .= $nivel2['numero'].". ".str_replace("&nbsp;", "",trim($nivel2['comentario']))."\n";
                                    break;
                                case 'Dificultad':
                                    $dificultad .= $nivel2['numero'].". ".str_replace("&nbsp;", "",trim($nivel2['comentario']))."\n";
                                    break;
                                case 'Observaciones':
                                    $observ .= $nivel2['numero'].". ".str_replace("&nbsp;", "",trim($nivel2['comentario']))."\n";
                                    break;
                                default:
                                    # code...
                                    break;
                        ...
                        foreach ($listadoPreg as $i) {
                            if ($resto[$i]=="Abierto") {
                                ...
                                $texto = $restot[$i];//"Abierto".
                            } else if ($resto[$i]=="NC") {
                                $fondo3='background-color:#ED1717; font-weight: bold; color:#fff;';
                                $texto=$restot[$i];//"No Cumple".
                            } else if ($resto[$i]=="C"){
                                $fondo3='background-color:#5EFF61;';
                                $texto = $restot[$i]; //"Cumple".
                            } else if ($resto[$i]=="NA") {
                                $fondo3='background-color:orange;';
                                $texto = $restot[$i]; //"No Aplica".
                            } else if ($resto[$i]=="Cerrado"){
                                $fondo3='background-color:green;';
                                $texto = $restot[$i]; //$resto[$i].
                            } else {
                                $fondo3="";
                            ...
                            echo '<td class="" style="">'.str_replace("&nbsp;", "", $logros).'</td>';
                            echo '<td class="" style="">'.str_replace("&nbsp;", "",$dificultad).'</td>';
                            echo '<td class="" style="">'.str_replace("&nbsp;", "",$observ).'</td>';
                            if($nivel1['incumplimiento_grave']==1) { $oig="SI"; } else { $oig="NO" ;}
                            echo '<td class="" style="">'.$oig.'</td>';
                            echo '<td class="" style="">'.trim($nivel1['descripcion_incumplimiento_grave']).'</td>';
                            echo '<td class="" style="">'.$nivel1['logros_prestador'].'</td>';
                            echo '<td class="" style="">'.$nivel1['dificultades_prestador'].'</td>';
                            echo '<td class="" style="">'.$nivel1['debido_proceso'].'</td>';
                            echo '<td class="" style="">'.$nivel1['asistencia_tecnica'].'</td>';
                            echo '<td class="" style="">'.$nivel1['otras_observaciones'].'</td>';
